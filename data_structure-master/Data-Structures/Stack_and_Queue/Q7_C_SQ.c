...
/* 모든 여는 괄호에 대응하는 닫는 괄호가 있고, 괄호의 순서가 올바르고, (먼저 열린 괄호가 나중에 닫혀야 함), 괄호 타입이 매칭 되는지 확인허는 함수.
	- 입력: 문자열 (괄호들: `()`, `[]`, `{}`)
	- 출력: 균형 잡혀있으면 1(또는 참), 아니면 0(또는 거짓)
*/
int balanced(char *expression)
{
	// 스택을 초기화한다.
	...
	for (int i = 0; expression[i] != '\0'; i++)
	{
		...
		// 여는 괄호에 해당되면 스택에 push.
		if (ch == '{' || ch == '[' || ch == '(')
		{
			push(&s, ch);
		}
		// 닫는 괄호에 해당되면 스택에서 여는 괄호를 꺼내서 서로 대응되는지 확인한다.
		else if (ch == '}' || ch == ']' || ch == ')')
		{
			// 스택이 비어있으면 매칭할 여는 괄호가 없음
			if (isEmptyStack(&s))
			{
				return 0; // 균형잡히지 않음
			}

			char openingBracket = pop(&s);
	...
	return isEmptyStack(&s); // 참이면 1, 거짓이면 0을 리턴하는 함수
}
