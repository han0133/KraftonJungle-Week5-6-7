...
/*
    두 트리가 완전히 비어있거나,
    루트 값, 왼쪽 자식 트리, 오른쪽 자식 트리가 모두 똑같이 생겼을 때만 동일하다고 판단하는 재귀 함수
    동일하면 return 1, 그렇지 않으면 return 0
*/
int identical(BTNode *tree1, BTNode *tree2)

{
    // 두 트리가 모두 비어있으면 동일하다. (종료 조건)
    if (tree1 == NULL && tree2 == NULL)
        return 1;
    // 두 트리 중 하나만 비어있거나 루트 값이 다르면 동일하지 않다.
    else if (tree1 == NULL || tree2 == NULL || tree1->item != tree2->item)
        return 0;
    // 두 트리의 서브트리가 모두 동일한지 재귀적으로 확인한다.
