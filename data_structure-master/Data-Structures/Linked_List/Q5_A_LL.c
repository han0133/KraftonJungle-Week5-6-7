...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	if (ll == NULL)
	{
		printf("에러: 원본 리스트가 NULL입니다\n");
		return;
	}

	if (resultFrontList == NULL || resultBackList == NULL)
	{
		printf("에러: 결과 리스트가 NULL입니다\n");
		return;
	}
	...
	int frontSize = (totalSize + 1) / 2; // 홀수면 앞쪽이 하나 더
	ListNode *pCur = ll->head;

	// Front 리스트 생성
	for (int i = 0; i < frontSize && pCur != NULL; i++)
	{
		int value = pCur->item;
		int result = insertNode(resultFrontList, resultFrontList->size, value);
		if (result != 0)
		{
			printf("Front 삽입 실패: 에러코드 %d\n", result);
			return;
		}
		pCur = pCur->next; // 다음 노드로 이동
	}

	// Back 리스트 생성
	for (int i = frontSize; i < totalSize && pCur != NULL; i++)
	{
		int value = pCur->item;
		int result = insertNode(resultBackList, resultBackList->size, value);
		if (result != 0)
		{
			printf("Back 삽입 실패: 에러코드 %d\n", result);
			return;
		}
		pCur = pCur->next; // 다음 노드로 이동
	}
